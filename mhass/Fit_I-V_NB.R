...
png("nb_model_fit.png", width = 8, height = 6, units = "in", res = 150)
par(mar = c(4, 4, 3, 1))

# Log-log plot
plot(I, V, log = "xy", pch = 16, cex = 0.3, col = rgb(0.5, 0.5, 0.5, 0.5),
     xlab = "Intensity (μ)", ylab = "Variability (CV²)", 
     main = "Negative Binomial Model Fit (log-log)",
...
), "nb_model_parameters.csv", row.names = FALSE)
...
), "variance_model_parameters.csv", row.names = FALSE)

# Save filtered data
write.csv(filtered_data, "R1_filtered_data.csv", row.names = FALSE)
